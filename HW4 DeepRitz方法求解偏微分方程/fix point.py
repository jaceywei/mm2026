...
# ------------------------------------------------------------------------------
def train_deep_ritz_fixed(
    input_dim=2, num_layers=3, hidden_dim=10, 
    num_iterations=50000, lr=1e-3, beta=1000.0, batch_size=1000
):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Training Slit Domain with FIXED points on device: {device}")
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=15000, gamma=0.1)
    loss_history = []
    
    # --- 核心修改：在训练开始前，一次性生成固定采样点 ---
    # 增加点的数量可以更公平地对比，因为固定点不再具备“遍历全域”的能力
    x_domain_fixed, x_boundary_fixed = sample_random_points_slit(
        input_dim=input_dim, batch_size=batch_size, domain=(-1.0, 1.0)
    )
    # 确保这些点不会在计算图中追踪梯度，但保持在 GPU 上
    x_domain_fixed = x_domain_fixed.detach()
    x_boundary_fixed = x_boundary_fixed.detach()
...
if __name__ == "__main__":
    # 执行训练
    num_layers = 4
    model_slit, loss_hist = train_deep_ritz_fixed(
