...
if __name__ == "__main__":
    ...
    # 1. 损失曲线对比
    plot_training_loss({k: v['loss'] for k, v in results1.items()}, title="Loss Comparison by Activation (Dataset 1)")
    plot_training_loss({k: v['loss'] for k, v in results2.items()}, title="Loss Comparison by Activation (Dataset 2)")
 
    # 2. 绘制决策边界
    plot_boundaries_separate(
        results1, (data1_train[0], data1_train[1]), (data1_test[0], data1_test[1]), 
        plot_target="Train", title_prefix="Dataset 1: Boundaries by Activation (TRAINING Set)"
    )
    plot_boundaries_separate(
        results1, (data1_train[0], data1_train[1]), (data1_test[0], data1_test[1]), 
        plot_target="Test", title_prefix="Dataset 1: Boundaries by Activation (TESTING Set)"
    )

    # --- 数据集 2 (Noisy) ---
    plot_boundaries_separate(
        results2, (data2_train[0], data2_train[1]), (data2_test[0], data2_test[1]), 
        plot_target="Train", title_prefix="Dataset 2: Boundaries by Activation (TRAINING Set)"
    )
    plot_boundaries_separate(
        results2, (data2_train[0], data2_train[1]), (data2_test[0], data2_test[1]), 
        plot_target="Test", title_prefix="Dataset 2: Boundaries by Activation (TESTING Set)"
    )

    # 3. 打印详细的性能对比表
    print("\n" + "="*85)
    print(f"{'Activation':<20} | {'Train Acc':<12} | {'Test (60)':<12} | {'Test (150)':<12}")
    print("-" * 85)
